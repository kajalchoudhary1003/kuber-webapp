...
// Dummy validator (replace with actual schema-based validation if needed)
const validateData = async (data, isUpdate = false) => {
  if (isUpdate) {
    // For updates, only validate if Status is being changed to Inactive
    if (data.Status === STATUS.INACTIVE && !data.EndDate) {
      throw new Error('EndDate is required when changing status to Inactive');
    }
  } else {
...
const updateClientEmployee = async (id, updates) => {
  ...
  try {
    ...
    await validateData(updates, true); // Pass true to indicate this is an update
